...
public class TagController implements BaseRestController<TagRequestDto, TagModelDto, Long> {
		...
		@ApiResponses(value = {
						@ApiResponse(code = 202, message = "Successfully updated tag's information"),
						@ApiResponse(code = 401, message = "You are not authorized to view the resource"),
						@ApiResponse(code = 403, message = "Accessing the resource you were trying to reach is forbidden"),
						@ApiResponse(code = 404, message = "The resource you were trying to reach is not found"),
						@ApiResponse(code = 500, message = "Application failed to process the request")
		...
		@ApiOperation(value = "Patch tag's information", response = TagModelDto.class)
		@ApiResponses(value = {
						@ApiResponse(code = 200, message = "Successfully patched tag's information"),
						@ApiResponse(code = 401, message = "You are not authorized to view the resource"),
						@ApiResponse(code = 403, message = "Accessing the resource you were trying to reach is forbidden"),
						@ApiResponse(code = 404, message = "The resource you were trying to reach is not found"),
						@ApiResponse(code = 500, message = "Application failed to process the request")
		})
		public ResponseEntity<TagModelDto> patch(@PathVariable Long id, @Valid @RequestBody TagRequestDto patchRequest) {
				patchRequest.setId(id);
				return new ResponseEntity<>(tagService.update(patchRequest), HttpStatus.ACCEPTED);
		}
